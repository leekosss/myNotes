...
date: 2022-08-15 19:49:22
tags: HTTP&Tomcat&Servlet
categories: HTTP&Tomcat&Servlet
...
![1627031023395](HTTP-Tomcat-Servlet/1627031023395-1660564211479.png)
...
等学习完JavaWeb之后，同学们就可以使用Java语言开发我们上述所说的网站。而国内很多大型网站公司也是首选Java语言来解决web互联网相关的问题。那都有哪些公司的系统是使用Java语言的呢?
 ![](HTTP-Tomcat-Servlet/20210717183958532-1660564211479.png)
...
![1627031933553](HTTP-Tomcat-Servlet/1627031933553-1660564211479.png)
...
* 在日常上网的过程中，我们除了看到这些好看的页面以外，还会碰到很多动态内容，比如我们常见的百度登录效果:
  ![1627037814180](HTTP-Tomcat-Servlet/1627037814180-1660564211480.png)
...
* 动态资源虽然可以处理逻辑，但是当用户来登录百度的时候，就需要输入`用户名`和`密码`,这个时候我们就又需要解决的一个问题是，用户在注册的时候填入的用户名和密码、以及我们经常会访问到一些数据列表的内容展示(如下图所示)，这些数据都存储在哪里?我们需要的时候又是从哪里来取呢?
  ![1627038674340](HTTP-Tomcat-Servlet/1627038674340-1660564211480.png)

#### 1.2.4 数据库

* 数据库主要负责存储数据。
* 整个Web的访问过程就如下图所示:
  ![1627039320220](HTTP-Tomcat-Servlet/1627039320220-1660564211480.png)
...
![1627043194238](HTTP-Tomcat-Servlet/1627043194238-1660564211480.png)
...
![1627046235092](HTTP-Tomcat-Servlet/1627046235092-1660564211480.png)
...
![1627050004221](HTTP-Tomcat-Servlet/1627050004221-1660564211480.png)
...
* 请求体: POST请求的最后一部分，存储请求参数
  ...
  ![1627050930378](HTTP-Tomcat-Servlet/1627050930378-1660564211480.png)
...
![1627278511902](HTTP-Tomcat-Servlet/1627278511902-1660564211480.png)

使用IDEA打开

![1627278583127](HTTP-Tomcat-Servlet/1627278583127-1660564211480.png)

打开后，可以点击项目中的`html\19-表单验证.html`，使用浏览器打开，通过修改页面中form表单的method属性来测试GET请求和POST请求的参数携带方式。

![1627278725007](HTTP-Tomcat-Servlet/1627278725007-1660564211480.png)
...
![1627053710214](HTTP-Tomcat-Servlet/1627053710214-1660564211480.png)
...
![1627058356051](HTTP-Tomcat-Servlet/1627058356051-1660564211480.png)
...
![1627060368806](HTTP-Tomcat-Servlet/1627060368806-1660564211480.png)
...
![1627176045795](HTTP-Tomcat-Servlet/1627176045795-1660564211480.png)
...
![1627178001030](HTTP-Tomcat-Servlet/1627178001030-1660564211480.png)
...
* 打开`apache-tomcat-8.5.68`目录就能看到如下目录结构，每个目录中包含的内容需要认识下,
  ...
  ![1627178815892](HTTP-Tomcat-Servlet/1627178815892-1660564211480.png)
...
![1627179006011](HTTP-Tomcat-Servlet/1627179006011-1660564211480.png)

启动后，通过浏览器访问 `http://localhost:8080`能看到Apache Tomcat的内容就说明Tomcat已经启动成功。

![1627199957728](HTTP-Tomcat-Servlet/1627199957728-1660564211480.png)

==注意==: 启动的过程中，控制台有中文乱码，需要修改conf/logging.prooperties

![1627199827589](HTTP-Tomcat-Servlet/1627199827589-1660564211480.png)
...
![1627200509883](HTTP-Tomcat-Servlet/1627200509883-1660564211480.png)
...
* Tomcat的端口号取值范围是0-65535之间任意未被占用的端口，如果设置的端口号被占用，启动的时候就会包如下的错误
  ...
  ![1627200780590](HTTP-Tomcat-Servlet/1627200780590-1660564211480.png)

* Tomcat启动的时候，启动窗口一闪而过: 需要检查JAVA_HOME环境变量是否正确配置

![1627201248802](HTTP-Tomcat-Servlet/1627201248802-1660564211480.png)
...
* Tomcat部署项目： 将项目放置到webapps目录下，即部署完成。
  ...
  * 通过浏览器访问`http://localhost/hello/a.html`，能看到下面的内容就说明项目已经部署成功。
    ...
    ![1627201572748](HTTP-Tomcat-Servlet/1627201572748-1660564211480.png)
...
* 一般JavaWeb项目会被打包称==war==包，然后将war包放到Webapps目录下，Tomcat会自动解压缩war文件
  ...
  * 通过浏览器访问`http://localhost/haha/a.html`，能看到下面的内容就说明项目已经部署成功。
    ...
    ![1627201868752](HTTP-Tomcat-Servlet/1627201868752-1660564211480.png)
...
* Maven Web项目结构: 开发中的项目
  ...
  ![1627202865978](HTTP-Tomcat-Servlet/1627202865978-1660564211480.png)

* 开发完成部署的Web项目

  ![1627202903750](HTTP-Tomcat-Servlet/1627202903750-1660564211480.png)
...
1. 创建Maven项目
   ...
   ![1627227574092](HTTP-Tomcat-Servlet/1627227574092-1660564211481.png)

2. 选择使用Web项目骨架

   ![1627227650406](HTTP-Tomcat-Servlet/1627227650406-1660564211481.png)

3. 输入Maven项目坐标创建项目

   ![1627228065007](HTTP-Tomcat-Servlet/1627228065007-1660564211481.png)

4. 确认Maven相关的配置信息后，完成项目创建

   ![1627228413280](HTTP-Tomcat-Servlet/1627228413280-1660564211481.png)

5. 删除pom.xml中多余内容，只留下面的这些内容，注意打包方式 jar和war的区别

   ![1627228584625](HTTP-Tomcat-Servlet/1627228584625-1660564211481.png)

6. 补齐Maven Web项目缺失的目录结构，默认没有java和resources目录，需要手动完成创建补齐，最终的目录结果如下

   ![](HTTP-Tomcat-Servlet/1627228673162-1660564211481.png)
...
1. 创建Maven项目
   ...
   ![1627229111549](HTTP-Tomcat-Servlet/1627229111549-1660564211481.png)

2. 选择不使用Web项目骨架

   ![1627229137316](HTTP-Tomcat-Servlet/1627229137316-1660564211481.png)

3. 输入Maven项目坐标创建项目

   ![1627229371251](HTTP-Tomcat-Servlet/1627229371251-1660564211481.png)

4. 在pom.xml设置打包方式为war,默认是不写代表打包方式为jar

   ![1627229428161](HTTP-Tomcat-Servlet/1627229428161-1660564211481.png)

5. 补齐Maven Web项目缺失webapp的目录结构

   ![1627229584134](HTTP-Tomcat-Servlet/1627229584134-1660564211481.png)

6. 补齐Maven Web项目缺失WEB-INF/web.xml的目录结构

   ![1627229676800](HTTP-Tomcat-Servlet/1627229676800-1660564211481.png)
...
7. 补充完后，最终的项目结构如下:
   ...
   ![1627229478030](HTTP-Tomcat-Servlet/1627229478030-1660564211481.png)
...
![1627204022604](HTTP-Tomcat-Servlet/1627204022604-1660564211481.png)

> 3.掌握不使用骨架的方式创建Maven Web项目

![1627204076090](HTTP-Tomcat-Servlet/1627204076090-1660564211481.png)
...
1. 打开添加本地Tomcat的面板
   ...
   ![1627229992900](HTTP-Tomcat-Servlet/1627229992900-1660564211481.png)

2. 指定本地Tomcat的具体路径

   ![1627230313062](HTTP-Tomcat-Servlet/1627230313062-1660564211481.png)

3. 修改Tomcat的名称，此步骤可以不改，只是让名字看起来更有意义，HTTP port中的端口也可以进行修改，比如把8080改成80

   ![1627230366658](HTTP-Tomcat-Servlet/1627230366658-1660564211481.png)

4. 将开发项目部署项目到Tomcat中

   ![1627230913259](HTTP-Tomcat-Servlet/1627230913259-1660564211481.png)
...
5. 部署成功后，就可以启动项目，为了能更好的看到启动的效果，可以在webapp目录下添加a.html页面
   ...
   ![1627233265351](HTTP-Tomcat-Servlet/1627233265351-1660564211481.png)

6. 启动成功后，可以通过浏览器进行访问测试

   ![1627232743706](HTTP-Tomcat-Servlet/1627232743706-1660564211481.png)

7. 最终的注意事项

   ![1627232916624](HTTP-Tomcat-Servlet/1627232916624-1660564211481.png)

   

至此，IDEA中集成本地Tomcat进行项目部署的内容我们就介绍完了，整体步骤如下，大家需要按照流程进行部署操作练习。

![1627205657117](HTTP-Tomcat-Servlet/1627205657117-1660564211481.png)
...
![1627233963315](HTTP-Tomcat-Servlet/1627233963315-1660564211481.png)

==注意:==

* 如果选中项目并右键点击后，看不到Run Maven和Debug Maven，这个时候就需要在IDEA中下载Maven Helper插件，具体的操作方式为: File --> Settings --> Plugins --> Maven Helper ---> Install,安装完后按照提示重启IDEA，就可以看到了。

![1627234184076](HTTP-Tomcat-Servlet/1627234184076-1660564211481.png)
...
![1627234763207](HTTP-Tomcat-Servlet/1627234763207-1660564211481.png)
...
* Servlet是JavaEE规范之一，其实就是一个接口，将来我们需要定义Servlet类实现Servlet接口，并由web服务器运行Servlet
  ...
  ![1627234972853](HTTP-Tomcat-Servlet/1627234972853-1660564211481.png)
...
![1627236923139](HTTP-Tomcat-Servlet/1627236923139-1660564211481.png)
...
* 通过案例演示下上述的生命周期
  ...
  */
  ...
  ![1627239292226](HTTP-Tomcat-Servlet/1627239292226-1660564211481.png)
...
![1627240593506](HTTP-Tomcat-Servlet/1627240593506-1660564211482.png)
...
* 一个Servlet,可以配置多个urlPattern
  ...
  ![1627272805178](HTTP-Tomcat-Servlet/1627272805178-1660564211482.png)
...
* ==urlPattern配置规则==
  ...
  * 精确匹配
    ...
    ![1627273174144](HTTP-Tomcat-Servlet/1627273174144-1660564211482.png)
  ...
  * 目录匹配
    ...
    ![1627273184095](HTTP-Tomcat-Servlet/1627273184095-1660564211482.png)
  ...
  * 扩展名匹配
    ...
    ![1627273194118](HTTP-Tomcat-Servlet/1627273194118-1660564211482.png)
    ...
    ![1627274483755](HTTP-Tomcat-Servlet/1627274483755-1660564211482.png)

    2. 如果路径配置的是`*.do`,那么在*.do的前面不能加`/`,否则会报错

    ![1627274368245](HTTP-Tomcat-Servlet/1627274368245-1660564211482.png)

  * 任意匹配

    ![1627273201370](HTTP-Tomcat-Servlet/1627273201370-1660564211482.png)
